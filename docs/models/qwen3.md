...
| `--request-option speaker_embedding_in=<file.emb>` | path | not set | CustomVoice: load a raw float32 speaker embedding (the format `speaker_embedding_out` writes) as the voice, replacing `--voice-ref` and the encoder pass entirely. Enables stored-voice reuse and embedding arithmetic — mixing or morphing voices via slerp between vectors interpolates the voice smoothly (measured: monotone register morph male→female). |
| `qwen3_tts.custom_voice_icl` | bool | `false` | Experimental: clone CustomVoice speakers through the Base ICL prompt — reference audio codes plus transcript in context instead of the embedding row alone. Needs `--voice-ref` **and** `--reference-text`, speech-tokenizer encoder weights in the package, and a reference in the target language (a cross-language reference wrecks pronunciation). Fixes the per-chunk pitch sink of embedding-only cloning (measured: within-sentence drift −4.0 → +0.6 semitones, register centered on the reference), but dampens instruction following — style instructs largely stop working. |
| `qwen3_tts.register_primer` | bool | `false` | Experimental register priming for CustomVoice: one canonical carrier take per voice/instruct/language is generated once (fixed seed, cached) and force-fed as the start of every chunk's generation, then trimmed from the audio. Keeps the instruct-capable prompt untouched — style instructions stay fully effective — while every chunk starts in the same acoustic state (measured: register spread across chunks 8.8 → 5.2 semitones). Does not prevent within-chunk declination. |
| `qwen3_tts.primer_text` | string | `"Alright,"` | Carrier text for register priming. Must end with continuing prosody (comma); a sentence-final carrier invites the model to stop right after the primed frames. |
...
- A cloned voice (`--voice-ref`) resolves through the speaker-embedding cache: the encoder runs once per distinct reference clip, and every later request or chunk with the same clip reuses the vector. Requests with different speakers, cloned voices, and instructions can share one batch.
