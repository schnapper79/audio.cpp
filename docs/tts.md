...
| `--session-option higgs_audio_tts.tail_cleanup=true|false` | bool | `true` | Trim the breath-like noise burst the model sometimes emits between the last word and end-of-audio (speech decays, an unvoiced high-frequency burst rises again, then silence). Word endings including final fricatives are preserved. |
| `--session-option higgs_audio_tts.tail_cleanup_fade_ms=<ms>` | 0-500 | `80` | How much of the detected burst is kept as a fade-out. `0` cuts hard (maximal cleanup, may clip an utterance-final fricative fused with a burst); larger values are more conservative. |
